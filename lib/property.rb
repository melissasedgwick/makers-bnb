class Property
  ...
  def self.update(id:, name:, description:, ppn:)
    result = DatabaseConnection.query("UPDATE properties SET name = '#{name}', description = '#{description}', price_per_night = '#{ppn}' WHERE id = #{id} RETURNING id, name, description, price_per_night, letter_id;")
    Property.new(id: result[0]['id'],
      name: result[0]['name'],
      description: result[0]['description'],
      ppn: result[0]['price_per_night'].to_i,
